  ...
  return (
    <Sheet>
      ...
      <SheetContent
        ...
        dir={lang === "ar" ? "rtl" : "ltr"}
        className="max-w-[600px] w-full  p-5 overflow-y-scroll"
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          {requiredFieldsRolesEdit?.map((e: any, i: number) => (
            <EachField title={e.name} data={e.values} />
          ))}
        </div>
        ...
        <div className="grid grid-cols-[1fr,1fr,1fr] max-[400px]:grid-cols-2 h-full gap-[5px] ">
          {data?.map((e: any, i: number) => (
            <span
              ...
              key={i}
              className={` ${
                values?.includes(e) ? "bg-primary text-white " : ""
              } flex items-center justify-center text-center  border-[1px] border-primary max-sm:p-[5px] max-sm:text-[12px] rounded-[6px] p-[10px] capitalize cursor-pointer hover:bg-primary hover:text-white duration-300`}
            >
              {" "}
              {t(e)}{" "}
            </span>
