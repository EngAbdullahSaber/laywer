...
import { Vendors, Dollar, Click } from "@/components/svg";
...
const ReportsSnapshot = () => {
  ...
  const tabsTrigger = [
    ...
    },
    {
      ...
      text: t("Reservations this month"),
      total: "125",
      color: "success",
    },
    {
        ...
        <div className="flex items-center gap-2 flex-wrap ">
          <div className="flex-1">
            ...
            <span className="text-xs text-default-600">
              {t("Demographic properties of your customer")}
            </span>
          </div>
          ...
          <TabsList className="grid grid-cols-2 lg:grid-cols-4 gap-2 md:gap-6 justify-start w-full bg-transparent h-full">
            {tabsTrigger.map((item, index) => (
              <TabsTrigger
                ...
                value={item.value}
                className={cn(
                  "flex flex-col gap-4 p-4 overflow-hidden   items-start  relative before:absolute before:left-1/2 before:-translate-x-1/2 before:bottom-1 before:h-[2px] before:w-9 before:bg-primary/50 dark:before:bg-primary-foreground before:hidden data-[state=active]:shadow-none data-[state=active]:before:block",
                  {
