...
const View = () => {
  ...
  // Lawyer details to be displayed in the sheet
  const lawyerDetails = [
    { label: t("Lawyer Name"), value: "Ahmed Ali Abdullah" },
    { label: t("Lawyer Category"), value: "جنائي", isHighlighted: true },
    { label: t("Phone Number"), value: "0581739548" },
    { label: t("Email Address"), value: "abc@gmail.com" },
          ...
          <ScrollArea className="h-[100%]">
            <div className="py-6">
              <ul className="md:grid grid-cols-2 !mt-5 gap-2 space-y-2 md:space-y-0">
                {lawyerDetails.map((detail, index) => (
                  <DetailItem
                    key={index}
                    label={detail.label}
                    value={detail.value}
                    // Adding a highlighted class for certain details
                    className={detail.isHighlighted ? "text-warning-700" : ""}
                  />
                ))}
              </ul>
            </div>
