...
import {
  getFilterServicesOrders,
  getServicesOrders,
  getServicesPanigationOrders,
  SearchServicesOrders,
...
const TableData = () => {
  ...
  const getClientData = async () => {
    ...
    if (queryString.length > 0) {
      try {
        const res = await getFilterServicesOrders(queryString, lang);
      ...
      try {
        const res =
          page === 1
            ? await getServicesOrders(lang)
            : await getServicesPanigationOrders(page, lang);
  ...
  const SearchData = async () => {
    ...
    try {
      const res = await SearchServicesOrders(search, lang);
